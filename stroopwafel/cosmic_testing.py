...
class Cosmic:
    ...
    def __init__(self, total_num_systems, num_batches_in_parallel, num_samples_per_batch, output_folder, output_filename, cosmic_filename, debug = False, run_on_helios = True, mc_only = False):
        ...
        self.evolve_time = 0
        self.reject_time = 0
        self.reject_systems_time = 0
        self.update_time = 0
        self.revert_time = 0
        self.sampling_time = 0
        self.dimensions_time = [0]
        self.rejecting_time = [0]
        self.fractional_time = 0
        self.misc_time = 0
    ...
    def process_batches_cosmic(self, batches, is_exploration_phase):
        for batch in batches:
            ...
            temp_df = pd.DataFrame({'bin_num': bin_nums, 
                                    'gaussian': 0})
            ...
            is_hit_map = dict(zip(bin_nums, [location.properties['is_hit'] for location in batch['samples']]))
            metallicity_map = dict(zip(bin_nums, batch['initC'].metallicity))
            
            batch['bpp']['gaussian'] = batch['bpp']['bin_num'].map(gaussian_map)
            batch['bpp']['generation'] = batch['bpp']['bin_num'].map(generation_map)
            batch['bpp']['is_hit'] = batch['bpp']['bin_num'].map(is_hit_map)
            batch['bpp']['metallicity'] = batch['bpp']['bin_num'].map(metallicity_map)
            batch['initC']['gaussian'] = batch['initC']['bin_num'].map(gaussian_map)
            batch['initC']['generation'] = batch['initC']['bin_num'].map(generation_map)
            ...
            printProgressBar(self.finished, self.total_num_systems, prefix = 'progress', suffix = 'complete', length = 20)
            print(len(bin_nums))
            print(len(batch['bpp'].bin_num.unique()))
            print(batch['bpp'].bin_num.max())
    ...
    def calculate_weights_of_samples(self):
        ...
        xPDF_time = 0
        read_distribution_time = 0
        append_time = 0
        for generation in range(NUM_GENERATIONS):
            read_distribution_start = time.time()
            distributions = self.read_distributions(generation + 1)
            read_distribution_end = time.time()
            read_distribution_time += read_distribution_end - read_distribution_start
        ...
        print("Reject systems time: ", self.reject_systems_time)
        print("Update properties time: ", self.update_time)
        print("Revert time: ", self.revert_time)
        print("Fractional time: ", self.fractional_time)
        print("Misc time: ", self.misc_time)
        print("Sampling time: ", self.sampling_time)
        print("Dimensions time: ", self.dimensions_time[0])
        print("Rejecting time: ", self.rejecting_time[0])
        ...
        print("Read distribution time: ", read_distribution_time)
        print("xPDF time: ", xPDF_time)
        print("---------------------------------")
        print("Total hits: ", self.num_hits)
        print("Sum of weights: ", np.sum(weights))
        mask_hits = np.asarray([location.properties['is_hit'] for location in locations])
        print("Sum of hit weights: ", np.sum(weights * mask_hits))
    ...
    def calculate_rejection_rate(self):
        ...
        for distribution in self.adapted_distributions:
            ...
            self.sampling_time += sampling_end - sampling_start
            misc_start = time.time()
            rejected = N_GAUSS - np.sum(mask)
            locations = np.asarray(locations)[mask]
            misc_end = time.time()
            self.misc_time += misc_end - misc_start
            revert_variables_start = time.time()
            [location.revert_variables_to_original_scales() for location in locations]
            revert_variables_end = time.time()
            self.revert_time += revert_variables_end - revert_variables_start
            update_properties_start = time.time()
            self.update_properties_method(locations, self.dimensions)
            update_properties_end = time.time()
            self.update_time += update_properties_end - update_properties_start
            self.reject_systems_start = time.time()
            rejected += self.rejected_systems_method(locations, self.dimensions, self.dimensions_time, self.rejecting_time)
            self.reject_systems_end = time.time()
            self.reject_systems_time += self.reject_systems_end - self.reject_systems_start
            fractional_start = time.time()
            fractional_rejected += rejected * distribution.alpha / N_GAUSS
            fractional_end = time.time()
            self.fractional_time += fractional_end - fractional_start
