...
from stroopwafel import genais, cosmic_testing, classes, prior, sampler, distributions, constants, utils
import argparse
...
def create_dimensions():
    ...
    q = classes.Dimension('q', 0.1, 1, sampler.uniform, prior.uniform)
...
def update_properties(locations, dimensions):
    ...
    for location in locations:
        ...
        location.properties['Metallicity_2'] = location.properties['Metallicity_1'] = location.dimensions[z]
        # location.properties['Metallicity_1'] = location.properties['Metallicity_2'] = metallicity
...
def interesting_systems_cosmic(batch):
    ...
    merge_times = evol.get_t_merge_ecc(ecc_i = interesting_systems_table.ecc.values, 
                                    a_i = interesting_systems_table.sep.values * u.Rsun, 
                                    m_1 = interesting_systems_table.mass_1.values * u.Msun, 
    ...
    return len(interesting_systems_table), interesting_systems_mask
...
def rejected_systems(locations, dimensions, dimensions_time=None, rejecting_time=None):
    """
    ...
    dimensions_end = time.time()
    if dimensions_time is not None:
        dimensions_time[0] += dimensions_end - dimensions_start
    # eccentricity = [location.properties['Eccentricity'] for location in locations]
    num_rejected = 0
    rejecting_loop_start = time.time()
    for index, location in enumerate(locations):
        ...
        if (mass_2[index] < constants.MINIMUM_SECONDARY_MASS):
            location.properties['is_rejected'] = 1
            num_rejected += 1
    rejecting_loop_end = time.time()
    if rejecting_time is not None:
        rejecting_time[0] += rejecting_loop_end - rejecting_loop_start
    return num_rejected
...
if __name__ == '__main__':
    ...
    for pair in pairs:
        # for metallicity in metallicities:
            pair = pair
            # metallicity = metallicity #For grid of metallicities

            # cosmic_filename = pair + '_' + str(metallicity)[2:] + '.h5'
            cosmic_filename = pair + '.h5'
            ...
            output_folder = os.path.join(os.getcwd(), 'output/' + pair + '/compas_test') # Folder you want to receieve outputs, here the current working directory, but you can specify anywhere
            ...
            cosmic_object = cosmic_testing.Cosmic(TOTAL_NUM_SYSTEMS, NUM_CPU_CORES, NUM_SYSTEMS_PER_RUN, output_folder, output_filename, cosmic_filename, debug = debug, run_on_helios = run_on_helios, mc_only = mc_only)
